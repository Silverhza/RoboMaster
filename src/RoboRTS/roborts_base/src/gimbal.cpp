...
void Gimbal::ROS_Init(){
  //ros publisher
  ros_pub_gimbal_gyro_ecd_angle_ = ros_nh_.advertise<roborts_msgs::GyroEcdAngle>("gimbal_gyro_ecd_angle", 1000);
  ...
  //ros_message_init
  gimbal_tf_.header.frame_id = car_id + "/base_link";
  gimbal_tf_.child_frame_id = car_id + "/gimbal";
  gimbal_tf_fixed.header.frame_id = car_id + "/base_link";
  gimbal_tf_fixed.child_frame_id = car_id + "/gimbal_fixed";
...
void Gimbal::GimbalInfoCallback(const std::shared_ptr<roborts_sdk::cmd_gimbal_info> gimbal_info){
  ...
  float tmp = gimbal_info->yaw_ecd_angle / 1800.0 * M_PI;
  gimbal_tf_fixed.header.stamp = current_time;
  gimbal_tf_fixed.transform.rotation = tf::createQuaternionMsgFromRollPitchYaw(0.0,
                                                                        0.0,
                                                                        tmp);
  gimbal_tf_fixed.transform.translation.x = 0;
  gimbal_tf_fixed.transform.translation.y = 0;
  gimbal_tf_fixed.transform.translation.z = 0.15;

  tf_broadcaster_.sendTransform(gimbal_tf_);
  tf_broadcaster_fixed.sendTransform(gimbal_tf_fixed);

  roborts_msgs::GyroEcdAngle angle_msg;
  angle_msg.gyro_angle = gimbal_info->yaw_gyro_angle / 1800.0 * M_PI;
  angle_msg.ecd_angle  = gimbal_info->yaw_ecd_angle  / 1800.0 * M_PI;
  ros_pub_gimbal_gyro_ecd_angle_.publish(angle_msg);
...
bool Gimbal::CtrlFricWheelService(roborts_msgs::FricWhl::Request &req,
  ...
  if(req.open){
    //fric_speed.left = 1450;  // 1350 -> 17
    //fric_speed.right = 1450;
    fric_speed.left = (unsigned int)req.left;  // 1350 -> 17
    fric_speed.right = (unsigned int)req.right;
  } else{
                              ...
                              roborts_msgs::ShootCmd::Response &res)
{
  ...
  //freq=1000-2500
  uint16_t default_freq = req.freq;
