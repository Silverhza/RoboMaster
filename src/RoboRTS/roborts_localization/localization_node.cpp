...
bool LocalizationNode::Init() 
{
  ...
  use_icp = localization_config.use_icp;
...
bool LocalizationNode::GetStaticNewMap()
{
  //rosbag::Bag bag;
  //std::string car_id;
  ...
  // if(car_id == "CAR1")
  //   bag.open("/home/one/roborts_ws/src/RoboRTS/roborts_localization/config/map.bag", rosbag::bagmode::Read);
  // else
  //   bag.open("/home/two/roborts_ws/src/RoboRTS/roborts_localization/config/map.bag", rosbag::bagmode::Read);
  // nav_msgs::OccupancyGrid MAP = *rosbag::View(bag,rosbag::TopicQuery("/map")).begin()->instantiate<nav_msgs::OccupancyGrid>();
  // amcl_ptr_->HandleMapMessage(MAP, init_pose_, init_cov_);
  static_map_srv_ = nh_.serviceClient<nav_msgs::GetMap>("static_map");
  ros::service::waitForService("static_map", -1);
  nav_msgs::GetMap::Request req;
  nav_msgs::GetMap::Response res;
...
void LocalizationNode::InitialPoseCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &init_pose_msg) 
{
  ...
  oldPose[0] = init_pose_mean[0];
  oldPose[1] = init_pose_mean[1];
  oldPose[2] = init_pose_mean[2];
  yaw_offset = init_pose_mean[2] - pose_in_odom[2];
  // Subtracting base to odom from map to base and send map to odom instead
  tf::Stamped<tf::Pose> odom_to_map;
  try 
  {
    tf::Transform tmp_tf(tf::createQuaternionFromYaw(init_pose_mean[2]),
                         tf::Vector3(init_pose_mean[0],
                                     init_pose_mean[1],
                                     0.0));
  ...
  static_tf_ = tf::Transform(tf::Quaternion(odom_to_map.getRotation()),
...
void LocalizationNode::LaserScanCallback(const sensor_msgs::LaserScan::ConstPtr &laser_scan_msg_ptr){
  ...
  amcl_ptr_->Update(pose_in_odom,
                    ...
                    particlecloud_msg_,
                    hyp_pose_);
    //hyp_pose_.pose_mean[2] =  0.2 * hyp_pose_.pose_mean[2] + 0.8 * (yaw_offset + pose_in_odom[2]);

    Vec3d delta;
    delta.setZero();
    delta[0] = hyp_pose_.pose_mean[0] - oldPose[0];
    delta[1] = hyp_pose_.pose_mean[1] - oldPose[1];
    delta[2] = std::cos(hyp_pose_.pose_mean[2] - oldPose[2]);
    // See if we should update the filter
    ifDrift = std::fabs(delta[0]) > 0.5 ||
        std::fabs(delta[1]) > 0.5 ||
        std::fabs(delta[2]) < 0.8;
  if(isFirst)
  {
    oldPose = hyp_pose_.pose_mean;
    isFirst = false;
  ...
  else
  {
    if(ifDrift)
    {
      std::cout<<" is drifted!!! "<<std::endl;
      hyp_pose_.pose_mean = oldPose;
    }
    else
    {
        oldPose = hyp_pose_.pose_mean;
  ...
  if(use_icp)
    for(int i=0;i<1;i++)
      icp->getFinal(laser_scan_msg_ptr,hyp_pose_.pose_mean);
...
bool LocalizationNode::PublishTf() 
{
  ...
  if(ifDrift)
  {
    tf::StampedTransform tmp_tf_stamped(static_tf_.inverse(),
  ...
  else if (amcl_ptr_->CheckTfUpdate()) 
  {
    // Subtracting base to odom from map to base and send map to odom instead
    tf::Stamped<tf::Pose> odom_to_map;
    try 
