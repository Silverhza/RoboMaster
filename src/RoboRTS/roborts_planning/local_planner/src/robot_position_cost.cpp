...
double RobotPositionCost::FootprintCost (const Eigen::Vector2d& position, const std::vector<Eigen::Vector2d>& footprint) {
    ...
    // ROS_WARN("[LOCAL PLANNER - POSITION COST] FootprintCost IS CALLED!!! FOOTPRINT: (%f, %f), (%f, %f), (%f, %f), (%f, %f), (%f, %f)", 
    // footprint[0].coeffRef(0), footprint[0].coeffRef(1), footprint[1].coeffRef(0), footprint[1].coeffRef(1),
    // footprint[2].coeffRef(0), footprint[2].coeffRef(1), footprint[3].coeffRef(0), footprint[3].coeffRef(1), 
    // footprint[4].coeffRef(0), footprint[4].coeffRef(1));

  unsigned int cell_x, cell_y;

  if (!costmap_.World2Map(position.coeffRef(0), position.coeffRef(1), cell_x, cell_y)) {
    // ROS_ERROR("[LOCAL PLANNER - POSITION COST] CONVERTING (%f, %f) FAILS", position.coeffRef(0), position.coeffRef(1));
  ...
  if (footprint.size() < 3) {
    ...
    if (cost == LETHAL_OBSTACLE  || cost == NO_INFORMATION /*|| cost == INSCRIBED_INFLATED_OBSTACLE*/) {
      // ROS_ERROR("[LOCAL PLANNER - POSITION COST] I AM RETURNED !");

      return -1.0;
    }
    // ROS_ERROR("[LOCAL PLANNER - POSITION COST] SIZE (%d) < 3 !", footprint.size() );
    return cost;
  }
  ...
  for(unsigned int i = 0; i < footprint.size() - 1; ++i) {
    if (!costmap_.World2Map(footprint[i].coeffRef(0), footprint[i].coeffRef(1), x0, y0)) {
      // ROS_ERROR("[LOCAL PLANNER - POSITION COST] CONVERTING (%f, %f) FAILS", footprint[i].coeffRef(0), footprint[i].coeffRef(1));
      return -1.0;
    }

    if(!costmap_.World2Map(footprint[i + 1].coeffRef(0), footprint[i + 1].coeffRef(1), x1, y1)) {
      // ROS_ERROR("[LOCAL PLANNER - POSITION COST] CONVERTING (%f, %f) FAILS", footprint[i + 1].coeffRef(0), footprint[i + 1].coeffRef(1));
      return -1.0;
    }
    ...
    if(line_cost < 0) {
      // ROS_ERROR("[LOCAL PLANNER - POSITION COST] UNFEASIBLE: LINE_COST = %f < 0 !", line_cost);
      return -1.0;
    }
  }

  if(!costmap_.World2Map(footprint.back().coeffRef(0), footprint.back().coeffRef(1), x0, y0)) {
    // ROS_ERROR("[LOCAL PLANNER - POSITION COST] UNFEASIBLE: CONVERTING (%f, %f) FAILS !", footprint.back().coeffRef(0), footprint.back().coeffRef(1));
    return -1.0;
  }

  if(!costmap_.World2Map(footprint.front().coeffRef(0), footprint.front().coeffRef(1), x1, y1)) {
    // ROS_ERROR("[LOCAL PLANNER - POSITION COST] UNFEASIBLE: CONVERTING (%f, %f) FAILS !", footprint.front().coeffRef(0), footprint.front().coeffRef(1));
    return -1.0;
  }
  ...
  // ROS_WARN("[LOCAL PLANNER - POSITION COST] footprint_cost: %f", footprint_cost);
...
double RobotPositionCost::FootprintCost(double x,
  ...
  // ROS_WARN("[LOCAL PLANNER - POSITION COST] ORIGINAL FOOTPRINT: (%f, %f), (%f, %f), (%f, %f), (%f, %f), (%f, %f)", 
  //   footprint_spec[0].coeffRef(0), footprint_spec[0].coeffRef(1), footprint_spec[1].coeffRef(0), footprint_spec[1].coeffRef(1),
  //   footprint_spec[2].coeffRef(0), footprint_spec[2].coeffRef(1), footprint_spec[3].coeffRef(0), footprint_spec[3].coeffRef(1), 
  //   footprint_spec[4].coeffRef(0), footprint_spec[4].coeffRef(1));
