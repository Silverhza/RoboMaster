...
ErrorInfo GlobalPlannerNode::Init() {
  ...
  ROS_WARN("[GLOBAL PLANNER] INITIALISING GLOBAL PLANNER ...");
  ...
  // SOS signal
  sos_pub_ = viz_nh.advertise<std_msgs::Empty>("/"+car_id+"/global_planner_sos",100);
  
  // Create global costmap
  std::string map_path = ros::package::getPath("roborts_costmap") + \
      "/config/costmap_parameter_config_for_global_plan_" + car_id + ".prototxt";
...
void GlobalPlannerNode::GoalCallback(const roborts_msgs::GlobalPlannerGoal::ConstPtr &msg) {
  ...
  while (ros::ok()) {
    ...
    if(node_state == NodeState::RUNNING || node_state == NodeState::SUCCESS || node_state == NodeState::FAILURE) {
      ...
      //   sos_pub_.publish(sos_signal);
      //   ROS_INFO("[GLOBAL PLANNER] SOS SIGNAL SENT ! HOPE FOR THE BEST !");
      // }
