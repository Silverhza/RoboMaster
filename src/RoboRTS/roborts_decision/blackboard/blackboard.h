...
void DebugCallback(const geometry_msgs::PoseStamped::ConstPtr& goal){
  ROS_WARN("[DECISION] DEBUG SIGNAL GOT !");
 ...
 public:
  ...
  explicit Blackboard(const std::string &proto_file_path,const std::string &jackal_ns):
    ...
    std::string map_path = ros::package::getPath("roborts_costmap") + \
      "/config/costmap_parameter_config_for_decision_" + car_id + ".prototxt";    
    ...
    // enemy_sub_ = rviz_nh.subscribe<geometry_msgs::PoseStamped>("test_goal", 1, &Blackboard::GoalCallback, this);
    ROS_WARN("[DECISION] GOAL SUBSCRIBTION IS NORMAL.");
  ...
  void GoalCallback(const geometry_msgs::PoseStamped::ConstPtr& goal){
    ROS_WARN("[DECISION] GOAL SIGNAL GOT !");
    new_goal_ = true;
    goal_ = *goal;
  }
