...
void debug_print_vector(vector<string> vec, string name){

	stringstream stringStream;
  	stringStream << to_string(vec.size()) << " ELEMENTS IN " << name << " : ";
  	for (auto& i : vec){
  		stringStream << i << ", ";
  	}
  	string copyOfStr = stringStream.str();

	ROS_WARN("[MERGER] %s", copyOfStr.c_str());
}
...
void LaserscanMerger::laserscan_topic_parser()
{
	...
	// debug_print_vector(tokens, "SET TOPICS (TOKEN)");
	// std::vector<string> topics_got;
	// for (auto& topic : topics){
	// 	topics_got.push_back(topic.name);
	// }
	
	// debug_print_vector(topics_got, "GOT TOPICS (TOPIC.NAME)");
	vector<string> tmp_input_topics;

	while (tmp_input_topics.size() != 2){
		tmp_input_topics.clear();
		...
		// debug_print_vector(tmp_input_topics, "TEMP INPUT VEC[0] (2)");
	}
	...
	// debug_print_vector(tmp_input_topics, "TEMP INPUT VEC[1] (2)");
	// debug_print_vector(input_topics, "INPUT VEC[0] (0)");
	...
	if( (tmp_input_topics.size() != input_topics.size()) || !equal(tmp_input_topics.begin(),tmp_input_topics.end(),input_topics.begin()))
	{
		...
		if(input_topics.size() > 0)
		{
            ...
            // debug_print_vector(input_topics, "INPUT VEC[1] (2)");
